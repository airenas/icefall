...
def main():
    ...
    total_lines = count_lines(args.lm_data)
    logging.info(f"Total number of lines in {args.lm_data}: {total_lines}")

    processed = 0

    with open(args.lm_data) as f:
        for line in tqdm(f, desc="Reading file", total=total_lines):
            line = line.strip()
            processed += 1
    ...
    for i in tqdm(range(num_sentences), desc="Computing sentence lengths", total=num_sentences, unit='sentences'):
