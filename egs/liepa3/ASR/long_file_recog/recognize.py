...
def get_parser():
    ...
    parser.add_argument(
        ...
        help="Path to file with chunks cuts.",
    )
    ...
    parser.add_argument(
        ...
        help="Path to file to save the chunk cuts with recognition results.",
    )
...
def decode_one_batch(
    ...
    encoder_out, encoder_out_lens = model.encoder(features=feature, feature_lengths=feature_lens)
...
def decode_dataset(
    ...
    for batch_idx, batch in enumerate(tqdm(dl, desc="Decoding")):
        ...
        _save_worker(cuts, hyps, timestamps, scores)
...
def run(rank, world_size, args, in_cuts):
    ...
    asr_data_module = Liepa3AsrDataModule(args)

    if world_size > 1:
        in_cuts = in_cuts[rank]
        out_cuts_filename = params.manifest_out + f"_job_{rank}"
    else:
...
def main():
    parser = get_parser()
    Liepa3AsrDataModule.add_arguments(parser)
    args = parser.parse_args()

    in_cuts_filename = Path(args.manifest_in)
