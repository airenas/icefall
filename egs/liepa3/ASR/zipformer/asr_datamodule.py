...
class Liepa3AsrDataModule:
    """
    DataModule for k2 ASR experiments.
    It assumes there is always one train and valid dataloader,
    but there can be multiple test dataloaders (e.g. Liepa3 test-clean
    and test-other).
    ...
    def train_cuts(self) -> CutSet:
        logging.info("mini_liepa3: About to get train cuts")
        return load_manifest_lazy(
            self.args.manifest_dir / "cuts_train.jsonl.gz"
        )

    @lru_cache()
    def train_shuf_cuts(self) -> CutSet:
        logging.info(
            "About to get the shuffled train cuts"
        )
        return load_manifest_lazy(
            self.args.manifest_dir / "cuts_train-shuf.jsonl.gz"
        )

    @lru_cache()
    def dev_cuts(self) -> CutSet:
        logging.info("mini_liepa3: About to get dev cuts")
        return load_manifest_lazy(
